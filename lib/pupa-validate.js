...
module.exports = function () {
  ...
  function validate_directory(directory, logger, finish) {
    ...
    if (matches) {
      ...
      glob('*.json', {
        ...
        for (_i = 0, _len = files.length; _i < _len; _i++) {
          validate(fs.readFileSync(path.join(directory, files[_i])), logger);
        }
        if (finish) {
          finish();
        }
  ...
  function validator(args, options, logger, finish) {
    ...
    register_schemas(urls, logger, function () {
      validate_directory(options.output_dir, logger, finish);
    });
  }
